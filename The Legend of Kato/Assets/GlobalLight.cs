...
public class GlobalLight : MonoBehaviour
{
    ...
    const float minG = .44f;
    const float maxG = .85f;

    const float minB = .44f;
    const float maxB = .85f;

    const float minIntensity = .6f;
    const float maxIntensity = .9f;

    const float minTimeGrad = 1f;
    const float maxTimeGrad = 3f;
